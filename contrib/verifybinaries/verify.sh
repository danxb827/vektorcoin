...
###   vektorcoincore.org and vektorcoin.org and compares them.
...
WORKINGDIR="/tmp/vektorcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://vektorcoincore.org"
HOST2="https://vektorcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="vektorcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'vektorcoin-',
...
if [ $? -ne 0 ]; then
   echo "vektorcoin.org failed to provide signature file, but vektorcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "vektorcoin.org and vektorcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the VEKTORCOIN Core binary release signing key installed?"
   fi
