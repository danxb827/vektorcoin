...
const char * const VEKTORCOIN_CONF_FILENAME = "vektorcoin.conf";
const char * const VEKTORCOIN_PID_FILENAME = "vektorcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "vektorcoind -foo=bar
...
#else
    const char* pszModule = "vektorcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VEKTORCOIN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VEKTORCOIN
    // Mac: ~/Library/Application Support/VEKTORCOIN
    // Unix: ~/.vektorcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VEKTORCOIN";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VEKTORCOIN";
#else
    // Unix
    return pathRet / ".vektorcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", VEKTORCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", VEKTORCOIN_PID_FILENAME)));
}
